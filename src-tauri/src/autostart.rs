...
//! login entry (`~/.config/autostart/WattDrive.desktop` on Linux, the `Run`
        ...
        None if cfg!(target_os = "linux") => {
            return Err(format!(
                "this instance is a bare binary ({}), not the installed AppImage. \
                 Start at login would point at it. Launch the installed WattDrive.AppImage \
                 and enable it there.",
                exe.display()
            ));
        }
...
pub fn repair_login_entry<R: Runtime>(app: &AppHandle<R>) {
    ...
    match app.autolaunch().enable() {
        Ok(()) => tracing::info!("rewrote the start-at-login entry to {}", target.display()),
        Err(e) => tracing::warn!("could not repair the start-at-login entry: {e}"),
    }
}
...
mod tests {
    ...
    fn the_installed_appimage_is_the_launch_target() {
        ...
        let installed = "/home/swatto/.local/lib/wattdrive/WattDrive.AppImage";
        let target = classify_linux(
            "/tmp/.mount_WattMaXYZ/usr/bin/wattdrive-desktop",
            Some(installed),
        );
    ...
    fn a_debug_binary_in_downloads_is_refused() {
        ...
        let err = classify_linux("/home/swatto/Downloads/wattdrive-desktop-0.14.8", None);
        ...
        let err = classify_linux(
            "/tmp/.mount_x/usr/bin/wattdrive-desktop",
            Some("/home/swatto/Downloads/WattDrive_0.15.0_amd64.AppImage"),
        );
    ...
    fn cargo_and_temp_builds_are_refused() {
        let err = classify_linux(
            "/home/swatto/Projects/WattDrive/src-tauri/target/debug/wattdrive-desktop",
            Some("/home/swatto/Projects/WattDrive/src-tauri/target/debug/wattdrive-desktop"),
        );
        assert!(err.unwrap_err().contains("development build"));
        let err = classify_linux(
            "/tmp/x/wattdrive-desktop",
            Some("/tmp/x/WattDrive.AppImage"),
        );
        assert!(err.unwrap_err().contains("temporary folder"));
        assert!(is_cargo_build(Path::new(
            "C:/src/WattDrive/src-tauri/target/release/wattdrive-desktop.exe"
        )));
        assert!(!is_cargo_build(Path::new(
            "/home/swatto/target/WattDrive.AppImage"
        )));
    }
    ...
    fn an_installed_exe_is_its_own_launch_target() {
        let exe = Path::new("C:/Users/swatto/AppData/Local/WattDrive/WattDrive.exe");
        let target = classify(
            exe,
            None,
    ...
    fn exec_path_reads_the_binary_from_the_desktop_entry() {
        let entry = "[Desktop Entry]\nType=Application\nVersion=1.0\nName=WattDrive\n\
                     Comment=WattDrivestartup script\n\
                     Exec=/home/swatto/Downloads/wattdrive-desktop-0.14.8 --hidden\n\
                     StartupNotify=false\nTerminal=false";
        assert_eq!(
            exec_path(entry),
            Some(PathBuf::from(
                "/home/swatto/Downloads/wattdrive-desktop-0.14.8"
            ))
        );
